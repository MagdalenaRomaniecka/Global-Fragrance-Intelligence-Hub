...
try:
    ...
    st.error(f"Data loading error. Please check your data files. Details: {e}")
    df = pd.DataFrame()
...
with tab1:
    ...
    with col_audio:
        ...
        if "Ep. 1" in selected_episode:
            ...
            # Bulletproof audio loading for Episode 1
            try:
                ...
                st.info("🎧 Audio track is currently unavailable. Please read the Executive Summary below.")
            ...
            # BULLETPROOF AUDIO LOADING (Bypasses GitHub LFS file errors)
            try:
                st.audio("podcast_2026.mp3")
            except Exception:
                st.info("🎧 Audio track is hosted on external LFS storage due to high fidelity. Please read the Executive Summary below.")
...
with tab2:
    ...
    if not df.empty:
        ...
        if 'price_usd' not in df_plot.columns:
            df_plot['price_usd'] = 100
