...
# -----------------------------------------------------------------------------
# 1. ATELIER SUPREME CSS - (ŚREDNIKI PRZYWRÓCONE DLA POPRAWNOŚCI KODU)
# -----------------------------------------------------------------------------
...
with tabs[0]:
    ...
    with col_audio:
        ...
        episode = st.radio("Selection:", ["🎧 Ep. 1: Recession Glam & 2025 Market", "🔮 Ep. 2: 2026 Outlook & AI Architecture", "🌍 Ep. 3: The European Barbell & Poland"], label_visibility="collapsed")
        ...
        if "Ep. 1" in episode:
            ...
            desc = "Analyzing Recession Glam, Sol de Janeiro dominance, and Russian import substitution strategies under sanctions."
            ...
            desc = "Strategic deep dive into the $5T AI era, the 2025 Tariff Shock, and the hollowing out of the middle tier."
            ...
            desc = "Mapping the European Barbell structure, Poland rising PPP, and the 0.28 omnichannel correlation paradox."
    ...
    with col_trans:
        with st.expander("📄 READ EXECUTIVE AUDIO DEBRIEF"):
            ...
            except: st.error("Debrief file missing.")
            ...
            with st.expander(rep_title):
                try:
                    ...
                    with open(target_r, 'r', encoding='utf-8') as f:
                        ...
                        st.markdown(f.read())
                        st.markdown('</div>', unsafe_allow_html=True)
                except: st.info("Report file missing.")
...
Market analysis reveals a clear Trickle Down Effect. Olfactory innovations typically debut in the Niche segment, where artistry and unique molecules are prioritized. Within 18 to 24 months, these same fragrance profiles are commercialized by Prestige houses. Ultimately, the trend reaches maturity in the Mass-Market segment, generating massive sales volumes through affordable alternatives.
</div>
</div>
    """, unsafe_allow_html=True)

    st.write("---")
    st.markdown('<div class="section-header">Strategic Trend Radar 2026 to 2030</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    t_list = [("🧪 Functional Scent", "AI designed neuro perfs for mental wellness."), ("🧛‍♀️ Vamp Romantic", "Gothic opulence and dark cherry dominance."), ("📈 Macro Resilience", "Poland rising economy and supply chain stability.")]
...
with tabs[2]:
    ...
    if f_choice != "-- Choose a Profile --":
        ...
        st.markdown(f"<div style='border: 2px solid #D4AF37; padding: 50px; background: #000; text-align: center;'><h1 style='color:#D4AF37; border:none;'>{f_data['name']}</h1><p style='color:#888;'>{f_data['brand']} • {f_data['segment']}</p><h2 style='color:#F0E68C; border:none;'>{f_data['community_score']:.1f}/5.0</h2><p style='color:#ccc;'>{f_data['top_notes']}</p></div>", unsafe_allow_html=True)

with tabs[3]:
    st.markdown('<div class="section-header">Analytical Project Ecosystem</div>', unsafe_allow_html=True)
    eco = [("🌍 Aromo Intelligence", "Russian market scraping engine.", "https://huggingface.co/spaces/Baphomert/Aromo-Market-Intelligence"), ("🔍 Perfume Finder", "Consumer recommendation PoC.", "https://perfume-finder-app-btskyvq7eytc5ujrgzr2dk.streamlit.app/")]
    for n, d, l in eco: st.markdown(f'<div class="project-card"><h4 style="color:#D4AF37;">{n}</h4><p style="color:#888;">{d}</p><a href="{l}" target="_blank" class="btn-launch">Launch</a></div>', unsafe_allow_html=True)
