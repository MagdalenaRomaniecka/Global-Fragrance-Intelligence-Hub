...
# -----------------------------------------------------------------------------
# 1. ATELIER SUPREME CSS (BALANCED LUXURY & FORCED CENTERING)
# -----------------------------------------------------------------------------
...
st.markdown("""
    ...
    /* HEADER FRAME - PERFECT CENTERING */
    ...
    .header-inner { border: 1px solid #D4AF37; padding: 25px 50px; text-align: center; background-color: #050505; box-shadow: inset 0 0 20px rgba(212,175,55,0.1); }
    ...
    /* GLOBAL HEADERS IN REPORTS */
    h1 { color: #D4AF37 !important; font-family: 'Tenor Sans' !important; text-align: center !important; border-bottom: 1px solid #D4AF37 !important; padding-bottom: 15px !important; text-transform: uppercase !important; font-size: 1.8rem !important; }
    h2 { color: #F0E68C !important; font-family: 'Tenor Sans' !important; text-align: center !important; text-transform: uppercase !important; border-top: 1px solid #333 !important; padding-top: 20px !important; margin-top: 35px !important; font-size: 1.4rem !important; }
    h3 { color: #D4AF37 !important; font-family: 'Tenor Sans' !important; border-left: 4px solid #D4AF37 !important; padding-left: 15px !important; font-size: 1.1rem !important; text-align: left !important; }
    ...
    /* REPORT CONTAINER */
    .report-frame { background: #080808; padding: 40px; border: 1px solid #222; box-shadow: 0 15px 40px rgba(0,0,0,0.6); color: #dfdfdf; line-height: 1.9; text-align: justify; font-size: 1.05rem; }

    /* VAULT CARD - CENTERING FIXED */
...
with tabs[0]:
    ...
    with col_audio:
        ...
        else: # Ep 2 and Ep 3 - GWARANCJA RAPORTU 2026
        ...
        st.audio(current_a); st.markdown(f'<p style="color:#888; font-size:0.9rem; font-style:italic; margin-top:20px; border-left: 3px solid #333; padding-left: 20px;">{desc}</p>', unsafe_allow_html=True)
...
with tabs[1]:
    ...
    fig_b = px.scatter(df, x="community_votes", y="community_score", size="price_usd", color="segment", hover_name="name", color_discrete_sequence=['#D4AF37', '#F0E68C', '#444'], template="plotly_dark", size_max=40)
    fig_b.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=650); st.plotly_chart(fig_b, use_container_width=True)
    st.markdown("""<div style="border: 1px solid #D4AF37; background: #080808; padding: 40px; margin-top: 40px; text-align: center;"><div style="color: #D4AF37; font-family: 'Tenor Sans'; font-size: 1.6rem; text-transform: uppercase; letter-spacing: 4px; margin-bottom: 25px; border-bottom: 1px solid #222; padding-bottom: 20px;">Strategic Insight: The Trickle-Down Effect</div><div style="color: #ccc; font-family: 'Lato'; font-size: 1.1rem; line-height: 1.9; text-align: justify;">Analiza rynkowa wykazuje wyraźny <strong>Trickle-Down Effect</strong>. Innowacje olfaktoryczne zazwyczaj debiutują w segmencie <strong>Niche</strong>, gdzie priorytetem jest artyzm i unikalne molekuły. W ciągu 18-24 miesięcy te same profile zapachowe są komercjalizowane przez domy <strong>Prestige</strong>. Ostatecznie trend osiąga fazę dojrzałości w segmencie <strong>Mass-Market</strong>, generując ogromne wolumeny sprzedaży poprzez przystępne cenowo alternatywy.</div></div>""", unsafe_allow_html=True)
...
with tabs[2]:
    ...
    st.markdown(f"""<div class="vault-card"><div class="vault-title">{f_data['name']}</div><div style="font-family: 'Lato'; color: #888; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 5px; margin-bottom: 45px;">{f_data['brand']} • {f_data['segment']}</div><div style="display: flex; justify-content: center; gap: 80px; margin: 50px 0; flex-wrap: wrap;"><div><p style="color:#666; font-size:0.8rem; letter-spacing:3px; margin-bottom:15px;">QUALITY SCORE</p><h3 style="color:#F0E68C; font-family:'Tenor Sans'; font-size:2.5rem; margin:0; border:none!important;">{f_data['community_score']:.1f}/5.0</h3></div><div><p style="color:#666; font-size:0.8rem; letter-spacing:3px; margin-bottom:15px;">GLOBAL VOTES</p><h3 style="color:#F0E68C; font-family:'Tenor Sans'; font-size:2.5rem; margin:0; border:none!important;">{f_data['community_votes']}</h3></div></div><div style="border-top:1px solid #222; padding-top:40px; max-width:700px; margin:0 auto;"><p style="color:#D4AF37; font-size:0.9rem; font-weight:bold; letter-spacing:3px; margin-bottom:20px; text-transform:uppercase;">Olfactory Strategic Profile</p><p style="color:#ccc; font-size:1.3rem; line-height:1.8; font-style:italic;">{f_data['top_notes']}</p></div></div>""", unsafe_allow_html=True)

# TAB 4: OUTLOOK
with tabs[3]:
    st.markdown('<div class="section-header">Strategic Trend Radar 2026–2030</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    trends = [("🧪 Functional Scent", "AI-designed neuro-perfs designed for mental wellness. Scent moves from aesthetics to biotech wellness, integrating with smart-home systems for bio-feedback."), ("🧛‍♀️ Vamp Romantic", "The definitive shift toward gothic opulence. Dark cherry, leather i dymne nuty dominują w Gen Z prestige, wypierając minimalistyczne profile 'Clean Girl'."), ("📈 Macro Resilience", "Poland's rise as a top-tier European economy. Lokalna produkcja i regionalne huby stają się kluczem do odporności łańcucha dostaw w dobie protekcjonizmu.")]
    for col, (t, d) in zip([c1, c2, c3], trends):
        col.markdown(f'<div style="border:1px solid #333; background:rgba(10,10,10,0.95); padding:40px; border-left: 5px solid #D4AF37; height:100%;"><h4 style="color:#D4AF37; font-family:Tenor Sans; font-size:1.4rem; letter-spacing:2px; margin-bottom:20px; text-transform:uppercase;">{t}</h4><p style="color:#bbb; font-size:1.05rem; line-height:1.8;">{d}</p></div>', unsafe_allow_html=True)

# TAB 5: ECOSYSTEM
with tabs[4]:
    st.markdown('<div class="section-header">Analytical Project Ecosystem</div>', unsafe_allow_html=True)
    eco = [("🌍 Aromo Intelligence", "Russian market scraping engine and strategic dashboard for real-time regional trend monitoring.", "https://huggingface.co/spaces/Baphomert/Aromo-Market-Intelligence"), ("🔍 Perfume Finder", "Consumer recommendation PoC based on high-fidelity preference matching and AI olfactory mapping.", "https://perfume-finder-app-btskyvq7eytc5ujrgzr2dk.streamlit.app/"), ("📊 ScentSational Analytics", "Deep learning trend visualization and community mapping for global fragrance launches.", "https://scentsational-zbznjhgc4xv7faddappdc2b.streamlit.app/"), ("🧪 ScentSational LFS Hub", "Backend architecture for massive dataset management and olfactory data versioning.", "https://baphomert-scentsational-fragrantica-lfs2.hf.space/")]
    e_cols = st.columns(2)
