...
st.markdown("""
    ...
    /* Nagłówek główny - PEŁNE CENTROWANIE I ZŁOTA RAMKA */
    ...
    /* GLOBALNE NAGŁÓWKI (WYŚRODKOWANE) */
    ...
    /* STRATEGIC METRICS */
    .metric-box { border: 1px solid #222; background-color: #080808; padding: 25px; text-align: center; border-radius: 2px; }
...
with tabs[0]:
    ...
    with col_audio:
        ...
        else: # Ep 2 & 3 - GWARANCJA RAPORTU 2026
    ...
    with col_viz:
        ...
        if f_type == "Barbell":
            ...
            df_v = df.nlargest(10, 'community_votes').sort_values('community_votes', ascending=True)
            fig = px.bar(df_v, x="community_votes", y="name", orientation='h', color_discrete_sequence=['#D4AF37'], text="community_votes", template="plotly_dark")
...
# TAB 3: FRAGRANCE VAULT (ZŁOTA RAMKA I CENTROWANIE)
with tabs[2]:
...
# TAB 4: OUTLOOK (PEŁNE OPISY)
with tabs[3]:
    st.markdown('<div class="section-header">Strategic Trend Radar 2026–2030</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    trends = [
        ("🧪 Functional Scent", "AI-designed neuro-perfs designed for mental wellness. Scent moves from pure aesthetics to a critical biotech wellness pillar, integrating with smart-home systems for mood regulation."), 
        ("🧛‍♀️ Vamp Romantic", "The definitive shift toward gothic opulence. Dark cherry, leather, and smoked oud dominance in Gen Z prestige, replacing the previous decade's minimalist aesthetics."), 
        ("📈 Macro Resilience", "Poland's rise as a top-tier European economy. Local production and regional hubs become key to supply chain resilience amidst global trade shifts.")
    ]
    for col, (t, d) in zip([c1, c2, c3], trends):
        col.markdown(f'<div style="border:1px solid #333; background:rgba(10,10,10,0.95); padding:45px; border-left: 5px solid #D4AF37; height:100%;"><h4 style="color:#D4AF37; font-family:Tenor Sans; font-size:1.4rem; letter-spacing:2px; margin-bottom:20px; text-transform:uppercase;">{t}</h4><p style="color:#bbb; font-size:1.1rem; line-height:2;">{d}</p></div>', unsafe_allow_html=True)

# TAB 5: ECOSYSTEM (WSZYSTKIE 4 APLIKACJE)
with tabs[4]:
