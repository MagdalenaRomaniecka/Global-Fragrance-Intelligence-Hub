...
def load_and_merge_data() -> pd.DataFrame:
    ...
    note_templates = [
        ...
        'Black Cherry, Leather, Almond', 
        'Saffron, Amberwood, Jasmine', 
        'Lavender, Lemon, AI Molecules', 
        'Sandalwood, Cardamom'
    ]
    ...
    for b in brands:
        if b in ['Zara', 'Lattafa', 'Armaf', 'Afnan', 'Missoni', 'Lalique', 'Faberlic', 'Novaya Zarya', 'Rasasi', 'Al Haramain', 'Zimaya', 'Maison Alhambra']: 
            segments.append('Mass-Market')
        elif b in ['Tom Ford', 'Creed', 'Xerjoff', 'Amouage', 'Roja outer', 'Roja Dove', 'Clive Christian', 'MFK', 'Le Labo', 'Byredo', 'Initio', 'LV', 'F. Malle', 'Nasomatto', 'Bvlgari']: 
    ...
    def classify_barbell_structure(price: float) -> str:
        if pd.isna(price): 
            return 'Unknown'
        if price < 80.0: 
