...
# -----------------------------------------------------------------------------
# 1. ATELIER SUPREME CSS - GLOBAL GOLD HIERARCHY & LUXURY DESIGN
# -----------------------------------------------------------------------------
    ...
    st.markdown("""
        <div style="border: 1px solid #D4AF37; background: #080808; padding: 40px; margin-top: 30px; border-radius: 2px; text-align: center;">
            ...
            <div style="color: #ccc; font-family: 'Lato'; font-size: 1.1rem; line-height: 1.9; text-align: justify;">
                Market data reveals a clear <strong>Trickle-Down Effect</strong>. Innovations typically originate in the <strong>Niche</strong> segment, prioritizing artistry and exotic molecules. Within 1-2 years, te profile are commercialized by <strong>Prestige</strong> houses. Finally, the trend reaches the <strong>Mass-Market</strong>, driving massive volume through affordable alternatives.
            </div>
        </div>
    """, unsafe_allow_html=True)
...
with tabs[2]:
    ...
    if f_choice != "-- Choose a Profile --":
        ...
        # FIX: Rounding the Quality Score to 1 decimal place
        rounded_score = round(f_data['community_score'], 1)
        st.markdown(f"""
            <div style="border: 1px solid #D4AF37; background: #050505; padding: 45px 25px; text-align: center; border-radius: 2px; box-shadow: 0 0 30px rgba(212,175,55,0.15);">
                ...
                <div style="display: flex; justify-content: center; gap: 70px; margin: 45px 0; flex-wrap: wrap;">
                    <div><p style="color:#666; font-size:0.75rem; letter-spacing:2px; margin-bottom:12px;">QUALITY SCORE</p><h3 style="color:#F0E68C; font-family:'Tenor Sans'; font-size:2.2rem; border:none !important; text-align:center !important;">{rounded_score}/5.0</h3></div>
...
with tabs[3]:
    ...
    t_list = [
        ("🧪 Functional Scent", "AI-designed neuro-perfs designed for mental wellness. Scent moves from aesthetics to biotech wellness."), 
        ...
        with e_cols[i % 2]: 
            st.markdown(f"""
                <div class="project-card">
                    <div><h4 style="color:#D4AF37; font-family:Tenor Sans; margin-bottom:12px; letter-spacing:1.5px;">{e_n}</h4><p style="color:#888; font-size:0.9rem; line-height:1.6;">{e_d}</p></div>
                    <div style="margin-top:30px;"><a href="{e_l}" target="_blank" class="btn-launch">🚀 Launch Professional Application</a></div>
                </div>
            """, unsafe_allow_html=True)
