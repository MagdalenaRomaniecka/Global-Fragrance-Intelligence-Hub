...
with tab1:
    ...
    with col_audio:
        ...
        selected_episode = st.radio("Select Episode:", [
            ...
            "🌍 Ep. 3: The European Barbell & Poland's Rise (2026)"
        ])
        ...
        elif "Ep. 3" in selected_episode:
            current_transcript_file = "ep3_whisper_transcript_EN.md"
            try:
                st.audio("ep3_europe_barbell.mp3", format="audio/mpeg")
            except Exception:
                st.warning("Please upload 'ep3_europe_barbell.mp3' to the repository.")
            st.markdown("""
                <div style="margin-top:20px; border-left:3px solid #D4AF37; padding:15px; background:rgba(212,175,55,0.05);">
                    <p style="color:#D4AF37; font-size:0.6rem; text-transform:uppercase; margin-bottom:5px; font-weight:bold;">Key Narrative</p>
                    <p style="color:#ccc; font-size:0.95rem; line-height:1.6; font-family:'Lato', sans-serif;">
                    The European fragrance market ($11.58B) is exhibiting a profound "Barbell" structure. Consumers are fleeing the 'middle market', transitioning to extreme budget or ultra-luxury. Poland serves as the primary hub with +75.3% growth.
                    </p>
                </div>
            """, unsafe_allow_html=True)
            current_filter = "Barbell"
            viz_title = "The Barbell Market Phenomenon"
    ...
    with col_viz:
        ...
        if not df.empty:
            ...
            # --- NEW: BARBELL CHART LOGIC FOR EP 3 ---
            if current_filter == "Barbell" and 'market_structure' in df.columns:
                ...
                fig = px.bar(
                    ...
                    title="Evidence of the Barbell Market Phenomenon",
                    color_discrete_map={'Ultra-Niche (Barbell Top)': '#D4AF37', 'Budget (Barbell Bottom)': '#F0E68C', 'Squeezed Middle': '#444444'},
                    template="plotly_dark"
                )
                fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_family="Lato", height=380, showlegend=False)
                st.plotly_chart(fig, use_container_width=True)
                
            # --- OLD: BUBBLE/BAR CHART LOGIC FOR EP 1 & 2 ---
            else:
    ...
    with col_doc2:
        report_label = "📈 READ 2025 TREND REPORT" if "Ep. 1" in selected_episode else "📈 READ 2026 MACRO REPORT"
        report_file = 'trend_report_2025.md' if "Ep. 1" in selected_episode else 'macro_report_2026.md'
        ...
        with st.expander(report_label):
            try:
                ...
                st.markdown('<div class="transcript-box">Report unavailable. Please ensure file is in the directory.</div>', unsafe_allow_html=True)
