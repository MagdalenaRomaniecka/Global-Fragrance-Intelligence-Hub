...
# -----------------------------------------------------------------------------
# 1. ATELIER SUPREME CSS (FULL RESTORATION: GOLD FRAMES & CENTERING)
# -----------------------------------------------------------------------------
...
st.markdown("""
    ...
    .header-wrapper { display: flex; justify-content: center; padding: 40px 0 30px 0; }
    .header-outer { border: 1px solid #444; padding: 10px; display: inline-block; width: 100%; max-width: 700px; }
    ...
    /* GLOBAL GOLD HEADERS */
    h1 { color: #D4AF37 !important; font-family: 'Tenor Sans' !important; text-align: center !important; border-bottom: 1px solid #D4AF37 !important; padding-bottom: 15px !important; text-transform: uppercase !important; }
    h2 { color: #F0E68C !important; font-family: 'Tenor Sans' !important; text-align: center !important; text-transform: uppercase !important; border-top: 1px solid #333 !important; padding-top: 20px !important; }
    ...
    .metric-label { color: #666; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 3px; margin-bottom: 10px; font-weight: 700; }
    .metric-value { color: #F0E68C; font-family: 'Tenor Sans', sans-serif; font-size: 2.2rem; margin: 0; }
    
    /* VAULT CARD - FULL CENTERING & ROUNDED SCORE */
    .vault-card { border: 1px solid #D4AF37; background: #050505; padding: 50px 30px; text-align: center; border-radius: 2px; box-shadow: 0 0 40px rgba(212,175,55,0.2); margin: 0 auto; max-width: 900px; }
    .vault-title { font-family: 'Tenor Sans', sans-serif; color: #D4AF37; font-size: 2.6rem; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 15px; }
    ...
    .report-frame { background: #080808; padding: 45px; border: 1px solid #222; box-shadow: 0 15px 40px rgba(0,0,0,0.6); color: #dfdfdf; line-height: 2; text-align: justify; }
    ...
    <div class="header-wrapper">
        <div class="header-outer">
            <div class="header-inner">
                ...
                <div style="font-family: 'Lato'; color: #888; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 4px; margin-top: 15px;">Strategic Hub • Predictive Forecast 2026</div>
            </div>
        </div>
    </div>
""", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    with col_audio:
        ...
        episode = st.radio("Intelligence Briefing Series:", ["🎧 Ep. 1: Recession Glam & 2025 Market", "🔮 Ep. 2: 2026 Outlook & AI Architecture", "🌍 Ep. 3: The European Barbell & Poland"], label_visibility="collapsed")
        ...
        if "Ep. 1" in episode:
            ...
            desc = "Strategic deep dive into macroeconomic shifts and hollowing out of the middle tier."
...
with tabs[1]:
    ...
    # POPRAWKA: Przejrzysty wykres z wartościami i czytelną skalą
    ...
    st.markdown("""
        <div style="border: 1px solid #D4AF37; background: #080808; padding: 45px; margin-top: 40px; border-radius: 2px; text-align: center;">
            <div style="color: #D4AF37; font-family: 'Tenor Sans'; font-size: 1.7rem; text-transform: uppercase; letter-spacing: 4px; margin-bottom: 25px; border-bottom: 1px solid #222; padding-bottom: 20px;">Strategic Insight: The Trickle-Down Effect</div>
            <div style="color: #ccc; font-family: 'Lato'; font-size: 1.15rem; line-height: 2; text-align: justify;">
                Analiza rynkowa wykazuje wyraźny <strong>Trickle-Down Effect</strong>. Innowacje olfaktoryczne zazwyczaj debiutują w segmencie <strong>Niche</strong>, gdzie priorytetem jest artyzm i unikalne molekuły. W ciągu 18-24 miesięcy te same profile zapachowe są komercjalizowane przez domy <strong>Prestige</strong>. Ostatecznie trend osiąga fazę dojrzałości w segmencie <strong>Mass-Market</strong>, generując ogromne wolumeny sprzedaży poprzez przystępne cenowo alternatywy, co widać na powyższym wykresie jako skupisko głosów w dolnej części skali cenowej.
            </div>
        </div>
    """, unsafe_allow_html=True)
...
with tabs[2]:
    ...
    if f_choice != "-- Choose a Profile --":
        ...
        # POPRAWKA: Zaokrąglenie Quality Score
        st.markdown(f"""
            <div class="vault-card">
                ...
                <div style="display: flex; justify-content: center; gap: 80px; margin: 50px 0; flex-wrap: wrap;">
                    <div><p style="color:#666; font-size:0.8rem; letter-spacing:3px; margin-bottom:15px;">QUALITY SCORE</p><h3 style="color:#F0E68C; font-family:'Tenor Sans'; font-size:2.5rem; border:none !important; text-align:center !important;">{f_data['community_score']:.1f}/5.0</h3></div>
                    <div><p style="color:#666; font-size:0.8rem; letter-spacing:3px; margin-bottom:15px;">GLOBAL VOTES</p><h3 style="color:#F0E68C; font-family:'Tenor Sans'; font-size:2.5rem; border:none !important; text-align:center !important;">{f_data['community_votes']}</h3></div>
                </div>
...
with tabs[3]:
    ...
    t_list = [
        ("🧪 Functional Scent", "AI-designed neuro-perfs designed for mental wellness. Scent moves from aesthetics to biotech wellness, integrując się z systemami smart-home."), 
        ("🧛‍♀️ Vamp Romantic", "The definitive shift toward gothic opulence. Dark cherry, leather i dymne nuty dominują w Gen Z prestige, wypierając czyste profile 'Clean Girl'."), 
        ("📈 Macro Resilience", "Poland's rise as a top-tier European economy. Lokalna produkcja i regionalne huby logistyczne stają się kluczem do odporności łańcucha dostaw.")
    ]
    for col, (t_title, t_text) in zip([c1, c2, c3], t_list):
        col.markdown(f'<div style="border:1px solid #333; background:rgba(10,10,10,0.95); padding:40px; border-left: 5px solid #D4AF37; height:100%;"><h4 style="color:#D4AF37; font-family:Tenor Sans; letter-spacing:2px; margin-bottom:20px; text-transform:uppercase;">{t_title}</h4><p style="color:#bbb; font-size:1rem; line-height:1.8;">{t_text}</p></div>', unsafe_allow_html=True)

# --- TAB 5: ECOSYSTEM ---
with tabs[4]:
    st.markdown('<div class="section-header">Analytical Project Ecosystem</div>', unsafe_allow_html=True)
    # POPRAWKA: Wszystkie 4 aplikacje przywrócone
    ecosystem = [
        ("🌍 Aromo Intelligence", "Russian market scraping engine and strategic dashboard for regional trend monitoring.", "https://huggingface.co/spaces/Baphomert/Aromo-Market-Intelligence"), 
