...
st.markdown("""
    ...
    .header-wrapper { display: flex; justify-content: center; text-align: center; padding: 30px 0 10px 0; }
    .header-outer { border: 1px solid #444; padding: 8px; display: inline-block; width: 100%; max-width: 650px; }
    .header-inner { border: 1px solid #D4AF37; padding: 20px 40px; background-color: #050505; box-shadow: inset 0 0 20px rgba(212,175,55,0.1); }
    ...
    .metric-box { border: 1px solid #222; background-color: #080808; padding: 20px; text-align: center; transition: 0.3s; margin-bottom: 10px; }
    .metric-box:hover { border-color: #D4AF37; box-shadow: 0 0 15px rgba(212, 175, 55, 0.2); }
    .metric-label { color: #666; font-size: 0.6rem; text-transform: uppercase; letter-spacing: 2px; font-weight: 700; margin-bottom: 5px; }
    .metric-value { color: #F0E68C; font-family: 'Tenor Sans', sans-serif; font-size: 1.8rem; }

    .report-frame { background: #080808; padding: 40px; border: 1px solid #222; box-shadow: 0 10px 30px rgba(0,0,0,0.5); color: #dfdfdf; line-height: 1.8; text-align: justify; margin-bottom: 20px; font-size: 1.05rem; width: 100%; }
    .section-header { color: #D4AF37; font-family: 'Tenor Sans'; font-size: 1.3rem; border-left: 5px solid #D4AF37; padding-left: 15px; margin: 25px 0 15px 0; text-transform: uppercase; letter-spacing: 2px; }
    
    .project-card { border: 1px solid #222; background: rgba(15,15,15,0.95); padding: 20px; transition: 0.3s; height: 100%; display: flex; flex-direction: column; justify-content: space-between; margin-bottom: 15px; }
    .project-card:hover { border-color: #D4AF37; box-shadow: 0 0 15px rgba(212, 175, 55, 0.1); }
    .btn-launch { display: block; width: 100%; padding: 10px; background: #D4AF37 !important; color: #000 !important; text-align: center; font-weight: bold; text-transform: uppercase; font-size: 0.65rem; text-decoration: none; letter-spacing: 1px; }

    .footer { position: relative; width: 100%; background-color: #000; color: #444; text-align: center; padding: 20px; font-size: 0.6rem; border-top: 1px solid #111; letter-spacing: 2px; margin-top: 50px; }
    </style>
...
<div style="font-family: 'Lato'; color: #888; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 3px; margin-top: 8px;">Global Strategic Hub ✦ Predictive Forecast 2026</div></div></div></div>""", unsafe_allow_html=True)
...
with tabs[0]:
    col_audio, col_viz = st.columns([1, 1.2], gap="medium")
    with col_audio:
        ...
        if "Ep. 1" in episode:
            ...
            f_type, v_title, desc = "Barbell", "The Barbell Market Structure 2026", "Mapping the European Barbell structure and Poland PPP."
        else:
        ...
        st.audio(current_a)
        st.markdown(f'<p style="color:#D4AF37; font-size:0.9rem; font-style:italic; margin-top:15px; border-left: 3px solid #D4AF37; padding-left: 15px;">{desc}</p>', unsafe_allow_html=True)
    ...
    with col_viz:
        ...
        fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_family="Lato", height=400, margin=dict(b=80))
        st.plotly_chart(fig, use_container_width=True)

    st.write("---")
    # UKŁAD KOLUMNOWY (NA TELEFONIE STACKUJE SIĘ AUTOMATYCZNIE)
...
with tabs[2]:
    ...
    vault_html = f"""
    <div style="border: 2px solid #D4AF37; padding: 4px; background: #000; margin: 20px auto; max-width: 800px; box-shadow: 0 0 30px rgba(212,175,55,0.15);">
        <div style="border: 1px solid rgba(212,175,55,0.4); background: radial-gradient(circle at 50% 50%, #0a0a0a 0%, #000000 100%); padding: 40px 20px; text-align: center;">
            <div style="font-family: 'Tenor Sans', sans-serif; color: #D4AF37; font-size: 2.2rem; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 10px;">{f_data['name']}</div>
            <div style="color: #D4AF37; font-size: 0.8rem; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 30px;">{f_data['brand']} ✦ {f_data['segment']}</div>
            <div style="display: flex; justify-content: center; gap: 20px; flex-wrap: wrap;">
                <div style="border: 1px solid #D4AF37; background: #050505; padding: 20px; min-width: 200px;">
                    <div style="color:#D4AF37; font-size:0.7rem; letter-spacing:2px; margin-bottom:10px; text-transform:uppercase;">Quality Score</div>
                    <div style="color: #F0E68C; font-family: 'Tenor Sans'; font-size: 2.5rem;">{f_data['community_score']:.1f}/5.0</div>
                </div>
                <div style="border: 1px solid #D4AF37; background: #050505; padding: 20px; min-width: 200px;">
                    <div style="color:#D4AF37; font-size:0.7rem; letter-spacing:2px; margin-bottom:10px; text-transform:uppercase;">Key Notes</div>
...
with tabs[3]:
    ...
    # TUTAJ MOŻESZ WKLEIĆ SWOJE LINKI W MIEJSCE href="https://..."
    apps = [
    ...
    for col, (name, desc, link) in zip([e1, e2, e3, e4], apps):
        col.markdown(f"""<div class="project-card"><h4 style="color:#D4AF37; margin-top:0; font-size:0.9rem;">{name}</h4><p style="color:#888; font-size:0.75rem;">{desc}</p><a class="btn-launch" href="{link}" target="_blank">LAUNCH APP</a></div>""", unsafe_allow_html=True)
