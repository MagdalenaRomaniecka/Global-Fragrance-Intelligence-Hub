...
# -----------------------------------------------------------------------------
# 1. ATELIER SUPREME CSS - INSPIRED BY SCENTSATIONAL (CENTERED & GOLD)
# -----------------------------------------------------------------------------
...
st.markdown("""
    ...
    /* GLOBAL HEADERS (CENTERED) */
    h1 { color: #D4AF37 !important; font-family: 'Tenor Sans' !important; text-align: center !important; border-bottom: 1px solid #D4AF37 !important; padding-bottom: 15px !important; text-transform: uppercase !important; letter-spacing: 3px !important; }
    h2 { color: #F0E68C !important; font-family: 'Tenor Sans' !important; text-align: center !important; text-transform: uppercase !important; border-top: 1px solid #333 !important; padding-top: 20px !important; letter-spacing: 2px !important; }
    ...
    /* FRAGRANCE VAULT CARD - FULL SCENTSATIONAL STYLE */
    .vault-card { border: 1px solid #D4AF37; background: #050505; padding: 60px 40px; text-align: center; border-radius: 2px; box-shadow: 0 0 40px rgba(212,175,55,0.15); margin: 30px auto; max-width: 900px; }
    ...
    .vault-subtitle { font-family: 'Lato', sans-serif; color: #888; font-size: 1rem; text-transform: uppercase; letter-spacing: 6px; margin-bottom: 45px; }

    /* REPORT CONTAINERS */
    .report-frame { background: #080808; padding: 45px; border: 1px solid #222; box-shadow: 0 15px 40px rgba(0,0,0,0.6); color: #dfdfdf; line-height: 2; text-align: justify; margin-bottom: 30px; font-size: 1.1rem; }
    
    /* PROJECT TILES (ECOSYSTEM) */
    ...
    @media (max-width: 768px) {
        .main-title { font-size: 1.8rem; letter-spacing: 3px; }
...
with tabs[0]:
    ...
    with col_audio:
        ...
        episode = st.radio("Series Selection:", ["🎧 Ep. 1: Recession Glam & 2025 Market", "🔮 Ep. 2: 2026 Outlook & AI Architecture", "🌍 Ep. 3: The European Barbell & Poland"], label_visibility="collapsed")
        ...
        if "Ep. 1" in episode:
            ...
            report_f, f_type, v_title = "macro_report_2026.md", ("None" if "Ep. 2" in episode else "Barbell"), ("2026 Projections" if "Ep. 2" in episode else "The Barbell Structure")
    ...
    with col_viz:
        ...
        fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_family="Lato", height=450, margin=dict(l=20, r=20, t=20, b=20))
        ...
        with st.expander(r_label):
            try:
                with open(report_f, 'r', encoding='utf-8') as f:
                    st.markdown('<div class="report-frame">', unsafe_allow_html=True); st.markdown(f.read()); st.markdown('</div>', unsafe_allow_html=True)
            except: st.info(f"Report '{report_f}' not found in folder.")
...
with tabs[1]:
    ...
    st.markdown("""
        <div style="border: 1px solid #D4AF37; background: #080808; padding: 45px; margin-top: 40px; border-radius: 2px; text-align: center;">
            ...
            <div style="color: #ccc; font-family: 'Lato'; font-size: 1.2rem; line-height: 2; text-align: justify; max-width: 1000px; margin: 0 auto;">
...
with tabs[2]:
    ...
    if f_choice != "-- Choose a Profile --":
        ...
        # FIX: ScentSational-style Centered Result
        st.markdown(f"""
            <div class="vault-card">
                ...
                <div style="display: flex; justify-content: center; gap: 80px; margin: 50px 0; flex-wrap: wrap;">
                    <div>
                        <p style="color:#666; font-size:0.9rem; letter-spacing:3px; margin-bottom:15px;">QUALITY SCORE</p>
                        <h3 style="color:#F0E68C; font-family:'Tenor Sans'; font-size:3.5rem; margin:0; border:none !important; text-align:center !important;">{f_data['community_score']:.1f}/5.0</h3>
                    </div>
                    <div>
                        <p style="color:#666; font-size:0.9rem; letter-spacing:3px; margin-bottom:15px;">GLOBAL VOTES</p>
                        <h3 style="color:#F0E68C; font-family:'Tenor Sans'; font-size:3.5rem; margin:0; border:none !important; text-align:center !important;">{f_data['community_votes']}</h3>
                    </div>
                </div>
                <div style="border-top:1px solid #222; padding-top:40px; max-width:750px; margin:0 auto;">
                    ...
                    <p style="color:#ccc; font-size:1.4rem; line-height:1.8; font-style:italic;">{f_data['top_notes']}</p>
                </div>
            </div>
        """, unsafe_allow_html=True)
...
with tabs[3]:
    ...
    trends = [
        ("🧪 Functional Scent", "AI-designed neuro-perfs designed for mental wellness. Scent moves from pure aesthetics to a critical biotech wellness pillar."), 
