...
# 1. ATELIER CSS - POWRÓT ZŁOTYCH RAMEK I CENTROWANIA
...
st.markdown("""
    ...
    /* ZŁOTY NAGŁÓWEK GŁÓWNY */
    .header-wrapper { display: flex; justify-content: center; padding: 40px 0; }
    .header-outer { border: 1px solid #444; padding: 10px; display: inline-block; width: 100%; max-width: 750px; }
    .header-inner { border: 1px solid #D4AF37; padding: 35px 70px; text-align: center; background-color: #050505; box-shadow: inset 0 0 30px rgba(212,175,55,0.1); }
    ...
    /* GLOBALNA TYPOGRAFIA RAPORTÓW */
    h1 { color: #D4AF37 !important; font-family: 'Tenor Sans' !important; text-align: center !important; border-bottom: 1px solid #D4AF37 !important; padding-bottom: 20px !important; text-transform: uppercase !important; margin-bottom: 30px !important; }
    ...
    /* VAULT CARD - PRZYWRÓCONE CENTROWANIE */
    ...
    .btn-launch { display:block; width:100%; padding:14px; background:#D4AF37 !important; color:#000 !important; text-align:center; font-weight:bold; text-transform:uppercase; font-size:0.75rem; text-decoration:none; letter-spacing: 1.5px; }
...
with tabs[1]:
    ...
    st.markdown("""<div style="border: 1px solid #D4AF37; background: #080808; padding: 45px; margin-top: 40px; text-align: center;"><div style="color: #D4AF37; font-family: 'Tenor Sans'; font-size: 1.8rem; text-transform: uppercase; letter-spacing: 4px; margin-bottom: 25px; border-bottom: 1px solid #222; padding-bottom: 20px;">Strategic Insight: The Trickle-Down Effect</div><div style="color: #ccc; font-family: 'Lato'; font-size: 1.2rem; line-height: 2; text-align: justify;">Analiza rynkowa wykazuje wyraźny <strong>Trickle-Down Effect</strong>. Innowacje olfaktoryczne zazwyczaj debiutują w segmencie <strong>Niche</strong>, gdzie priorytetem jest artyzm i unikalne molekuły. W ciągu 18-24 miesięcy te same profile zapachowe są komercjalizowane przez domy <strong>Prestige</strong>. Ostatecznie trend osiąga fazę dojrzałości w segmencie <strong>Mass-Market</strong>, generując ogromne wolumeny sprzedaży poprzez przystępne cenowo alternatywy.</div></div>""", unsafe_allow_html=True)
...
with tabs[2]:
    ...
    if f_choice != "-- Choose a Profile --":
        ...
        st.markdown(f"""<div class="vault-card"><div class="vault-title">{f_data['name']}</div><div style="font-family: 'Lato'; color: #888; font-size: 1rem; text-transform: uppercase; letter-spacing: 5px; margin-bottom: 45px;">{f_data['brand']} • {f_data['segment']}</div><div style="display: flex; justify-content: center; gap: 90px; margin: 50px 0; flex-wrap: wrap;"><div><p style="color:#666; font-size:0.9rem; letter-spacing:3px; margin-bottom:15px;">QUALITY SCORE</p><h3 style="color:#F0E68C; font-family:'Tenor Sans'; font-size:3rem; margin:0;">{f_data['community_score']:.1f}/5.0</h3></div><div><p style="color:#666; font-size:0.9rem; letter-spacing:3px; margin-bottom:15px;">GLOBAL VOTES</p><h3 style="color:#F0E68C; font-family:'Tenor Sans'; font-size:3rem; margin:0;">{f_data['community_votes']}</h3></div></div><div style="border-top:1px solid #222; padding-top:40px; max-width:750px; margin:0 auto;"><p style="color:#D4AF37; font-size:1rem; font-weight:bold; letter-spacing:3px; margin-bottom:20px; text-transform:uppercase;">Olfactory Strategic Profile</p><p style="color:#ccc; font-size:1.4rem; line-height:2; font-style:italic;">{f_data['top_notes']}</p></div></div>""", unsafe_allow_html=True)
...
with tabs[3]:
    ...
    t_list = [
        ("🧪 Functional Scent", "AI-designed neuro-perfs designed for mental wellness. Scent moves from aesthetics to biotech wellness, integrując się z systemami bio-feedbacku."), 
        ("🧛‍♀️ Vamp Romantic", "The definitive shift toward gothic opulence. Dark cherry, leather i dymne nuty dominują w Gen Z prestige, wypierając minimalistyczne profile."), 
        ("📈 Macro Resilience", "Poland's rise as a top-tier European economy. Lokalna produkcja i regionalne huby stają się kluczem do odporności łańcucha dostaw.")
    ]
