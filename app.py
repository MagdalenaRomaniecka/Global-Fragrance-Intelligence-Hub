...
st.markdown("""
    ...
    /* --- LUXURY REPORT BOX & GOLD HIERARCHY --- */
    .transcript-box { 
    ...
    /* Aggressive Gold CSS for Markdown Headers */
    .transcript-box h1 { 
    ...
    .btn-launch:hover { background: #F0E68C !important; }

    .section-header { color: #D4AF37; font-family: 'Tenor Sans', sans-serif; font-size: 1.4rem; border-left: 4px solid #D4AF37; padding-left: 18px; margin: 35px 0 20px 0; text-transform: uppercase; letter-spacing: 2px; }
...
with tabs[0]:
    ...
    with col_audio:
        ...
        # LOGIC: Ensure Episode 2 and 3 both point to the 2026 Macro Report
        if "Ep. 1" in episode:
            ...
            f_type, v_title, desc = "None", "2026 Global Projections", "Deep dive into AI architecture, NVIDIA dominance, and neuro-perfumery."
        else: # Ep 3
    ...
    # SANDWICH METHOD: Force CSS box around Markdown-rendered content
    with d1:
        ...
        with st.expander(report_label):
            try:
                with open(report_f, 'r', encoding='utf-8') as f:
                    ...
                    st.markdown(f.read())
                    st.markdown('</div>', unsafe_allow_html=True)
            except: st.info(f"The strategic report '{report_f}' was not found in the directory.")
