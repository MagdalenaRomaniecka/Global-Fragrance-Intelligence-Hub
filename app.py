...
with tabs[0]:
    ...
    with col_viz:
        ...
        if f_type == "Barbell":
            ...
            fig = px.bar(b_counts, x='Tier', y='Count', color='Tier', template="plotly_dark")
            ...
            df_t = df.nlargest(10, 'community_votes')
            fig = px.bar(df_t, x="community_votes", y="name", orientation='h', template="plotly_dark")
            fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_family="Lato", height=450)
            st.plotly_chart(fig, use_container_width=True)
...
with tabs[1]:
    ...
    fig_sun = px.sunburst(df_sunburst, path=[px.Constant("Global Market"), 'segment', 'brand', 'name'], values='community_votes', color='segment', template="plotly_dark")
