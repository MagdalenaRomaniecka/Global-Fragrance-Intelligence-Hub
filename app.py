...
st.markdown("""
    ...
    /* GLOBAL HEADERS IN REPORTS - FORCED CENTERING */
    ...
    .vault-card { border: 1px solid #D4AF37; background: #050505; padding: 60px 40px; text-align: center; border-radius: 2px; box-shadow: 0 0 40px rgba(212,175,55,0.15); margin: 20px auto; max-width: 850px; }
    ...
    .report-frame { background: #080808; padding: 45px; border: 1px solid #222; box-shadow: 0 15px 40px rgba(0,0,0,0.6); color: #dfdfdf; line-height: 2; text-align: justify; margin-bottom: 30px; font-size: 1.05rem; }
...
# TAB 2: MARKET ANALYTICS (NEW: Hierarchy Sunburst with IDM vibe)
with tabs[1]:
    st.markdown('<div class="section-header">Market Segmentation Strategic Hierarchy</div>', unsafe_allow_html=True)
    # FIX: Cleaner scatter plot, less mass, more professional
...
with tabs[2]:
    ...
    if f_choice != "-- Choose a Profile --":
        ...
        st.markdown(f"""<div class="vault-card"><div class="vault-title">{f_data['name']}</div><div style="font-family: 'Lato'; color: #888; font-size: 1rem; text-transform: uppercase; letter-spacing: 5px; margin-bottom: 45px;">{f_data['brand']} • {f_data['segment']}</div><div style="display: flex; justify-content: center; gap: 80px; margin: 50px 0; flex-wrap: wrap;"><div><p style="color:#666; font-size:0.9rem; letter-spacing:3px; margin-bottom:15px;">QUALITY SCORE</p><h3 style="color:#F0E68C; font-family:'Tenor Sans'; font-size:2.5rem; margin:0; border:none !important;">{f_data['community_score']:.1f}/5.0</h3></div><div><p style="color:#666; font-size:0.9rem; letter-spacing:3px; margin-bottom:15px;">GLOBAL VOTES</p><h3 style="color:#F0E68C; font-family:'Tenor Sans'; font-size:2.5rem; margin:0; border:none !important;">{f_data['community_votes']}</h3></div></div><div style="border-top:1px solid #222; padding-top:40px; max-width:750px; margin:0 auto;"><p style="color:#D4AF37; font-size:1rem; font-weight:bold; letter-spacing:3px; margin-bottom:20px; text-transform:uppercase;">Olfactory Strategic Profile</p><p style="color:#ccc; font-size:1.3rem; line-height:1.8; font-style:italic;">{f_data['top_notes']}</p></div></div>""", unsafe_allow_html=True)

# TAB 4: 2026 OUTLOOK
with tabs[3]:
    st.markdown('<div class="section-header">Strategic Trend Radar 2026–2030</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    trends = [("🧪 Functional Scent", "AI-designed neuro-perfs designed for mental wellness."), ("🧛‍♀️ Vamp Romantic", "Gothic opulence. Dark cherry and leather dominance in Gen Z prestige."), ("📈 Macro Resilience", "Poland's rise as a top-tier European economy.")]
...
with tabs[4]:
    ...
    eco = [("🌍 Aromo Intelligence", "Russian market scraping engine and strategic dashboard.", "https://huggingface.co/spaces/Baphomert/Aromo-Market-Intelligence"), ("🔍 Perfume Finder", "Consumer recommendation PoC based on preference matching.", "https://perfume-finder-app-btskyvq7eytc5ujrgzr2dk.streamlit.app/"), ("📊 ScentSational Analytics", "Deep learning trend visualization and community mapping.", "https://scentsational-zbznjhgc4xv7faddappdc2b.streamlit.app/"), ("🧪 ScentSational LFS Hub", "Backend architecture for massive dataset management.", "https://baphomert-scentsational-fragrantica-lfs2.hf.space/")]
    e_cols = st.columns(2)
